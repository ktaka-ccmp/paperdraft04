...
The configurations for the router is also summarised in \ref{appendix:router_config}.
...
%  For the green lines, a service IP address is used. The red lines use the IP addresses of the overlay network. The blue line uses the IP addresses of the node network.
  ...
  (a): The packets from Internet to a service IP in 10.1.1.0/24 is routed to the load balancer pod by set of routing rules shown in (b).
...
The traffic from the Internet is forwarded by ECMP routing table on the router to the node that hosts a load balancer pod, then routed to that pod by set of routing rules in Fig.\ref{fig:exabgp}~(\subref{fig:exabgp_setting}). 
And then the ipvs forwards them to IP addresses of nginx pods.
The IP address of any pod is dynamically assigned from 172.16.0.0/16 when the pod is started. 
...
On average the data size of each request and the corresponding response was in total about 636 [byte/req] including TCP/IP headers, ethernet header, and interframe gaps.
...
\caption{CPU usage of the ipvs and iptable DNAT.}
\label{fig:cpu_usage}
\end{figure}
