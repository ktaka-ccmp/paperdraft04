...
\subsection{Proposed architecture with a potable load balancer}
...
When the dynamic peering is supported, one can only specify possible peer IP address range instead of specifying every IP address for peers.
...
%
Running containers in the node net namaspace loses the whole point of containerization, i.e., they share the node network without separation.
This requires the users additional efforts to avoid VRRP conflict in configuration.
...
Here we discuss the implementation of actual proof of concept system for our proposed load balancers with ECMP redundancy in detail.
...
On the other hand, gobgp is used in the router and the route reflector, because exabgp did not seem to support add-path\cite{walton2016advertisement} needed for multi-path advertisement and FIB manipulation\cite{exa-networks_2018}.
...
Other BGP configurations are shown in \ref{appendix:exabgp_config},\ref{appendix:gobgpd_config_router},\ref{appendix:router_config}.
