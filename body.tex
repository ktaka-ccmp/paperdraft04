...
The saturated performance level indicates the maximum performance of the load balancer, which could be determined either by network bandwidth, hardware performance for load balancer or that for the benchmark client.
...
Both results show similar characteristics as the experiment in an on-premise data center in Fig.~\ref{fig:ipvs_performance}~(\subref{fig:ipvs-iptables-nginx}), where throughput increased linearly to a certain level that is determined by either network speed or machine specifications.
...
However, since there are cases in the cloud environment, where changing the VM types or CPU counts also changes the network bandwidth limit, a detailed analysis is further required in the future to clarify which factor limits the throughput.
Still, we can say that the load balancers can be run in GCP and AWS, and function properly until they reach the infrastructure limitations.
...
After that, we also measured the throughput when multiple of the ipvs container load balancers are deployed.

Fig.~\ref{fig:lb_ecmp_schem} shows the schematic diagram of the experimental setup and also summarizes hardware and software specifications.
Notable differences from the previous throughput experiment in Fig.~\ref{fig:benchmark-schem} are as follows;
1) Each load balancer pods now consists of both an ipvs container and an exbgp container.
2) The routing table at the router, which also acts as a benchmark client, is updated by BGP protocol through a route reflector.
3) The router's NIC has been changed to 10 Gbps card since now we have multiple of ipvs container load balancers that will use up 1 Gbps bandwidth.
4) All of the software has been updated to the most recent versions at the time of the experiment.

First, we examined ECMP functionality by watching the routing table on the router.
...
Fig.~\ref{fig:exabgp_routing_table}~(\subref{fig:double_svc}) shows the case where we additionaly started new service with two load balancer pods with service addresses in 10.1.2.0/24 range.
...
Fig.~\ref{fig:ecmp_delay_histgram_ieice}~(\subref{fig:ecmp_lb_cubic_ieice}) shows the results of the measurements.
There are four solid lines in the figure, each conrresponding the throughput result when there are one through four of the proposed load balancers.
...
The saturated levels, i.e. performane levels depend on the number of the ipvs load balancer pods(lb x 1 being the case with one ipvs pods, and lb x2 being two of them and as such), which increases linearly as we increases the number of the load balancers.
The dotted line in the figure show the throughput result when there were five load balancers. 
It had almost same paformance level as the case when there were four load balancers, and we suspect that this was due to the performance limit of the router as the bench mark client, since CPU idle was 0\%.
We expect that replacing the benchmark client with more powerfull machines will improve the performance level further. 

Fig.~\ref{fig:ecmp_delay_histgram_ieice}~(\subref{fig:ecmp_response_ieice}) shows the throughput measurement results when we periodicaly changed the number of the load balancers. 
...
As we can see from the figure, the blue line nicely follows the red line.

Fig.~\ref{fig:ecmp_delay_histgram_ieice}~(\subref{fig:ecmp_delay_histgram_ieice}) shows histgram of the ECMP update delay, where we measured the delays until the change in number of running pods is reflected in the routing table in the router during the experiment in Fig.~\ref{fig:ecmp_delay_histgram_ieice}~(\subref{fig:ecmp_response_ieice}) for 20 hours.
As we can see form the figure, most of the delays are within 7 seconds, and the largest delay during the 20 hours experiment was 10 seconds.
