...
In this paper, we extend the previous work and propose a software load balancer architecture with Equal Cost Multi-Path(ECMP)\cite{thaler2000multipath} redundancy by running a BGP agent container together with ipvs container.
...
When a container in the Fig.~\ref{fig:overlay} communicates with any node, it can use its IP address in 172.16.0.0/16 IP range as a source IP, since every node has proper routing table for the overlay network.
When a container communicates with the upstream router that does not have routes regarding the overlay network, the source IP address must be translated by Source Network Address Translation(SNAT) rules on the node the container resides.

The SNAT may cause problems when multiple containers on a single node try to connect the upstream router; the upstream router may not be able to distinguish the two separate connections from different containers, from the two separate connections from a single node.
This was the case in our experimental setups.
