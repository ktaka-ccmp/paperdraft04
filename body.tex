...
In this paper, we extend the previous work and propose a software load balancer architecture with Equal Cost Multi-Path(ECMP)\cite{thaler2000multipath} redundancy for container clusters.
...
We launch multiple of such pods and form a cluster of load balancers.
...
Major cloud providers do not currently provide BGP peering for users; therefore our proposed load balancer may lack ECMP redundancy in such environments and need to access cloud providers APIs to set up the route to the load balancer.
However, the authors hope they start providing BGP peering, once our proposed load balancer architecture is widely accepted as feasible.
