...
We implemented an experimental web cluster system with multiple of load balancers and web servers using Kubernetes and OSSs on top of standard Linux boxes to prove that the proposed architecture functions as intended. 
...
the Kubernetes cluster system while it showed the similar performance levels as the existing iptables DNAT based load balancer.
We also examined that the ECMP redundant feature functioned as intended by monitoring the routing table of the upstream router and packet flow as we periodically accessed the web cluster.
Over all our system functioned as we intended. 
For future work we plan to do performance mesurements for the scalability of our system and also improve performance of a single software load balancer on standard Linux box using Xpress Data Plane(XDP) technology. 
